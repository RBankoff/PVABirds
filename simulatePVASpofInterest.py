...
bird_data_infile = pd.read_excel(io='03_08_ 2022_PVA_Bird_Data_cleaned_species_of_interest.xlsx', engine='openpyxl')
    ...
    if pd.isna(InitialPopulationSizeMasoalaPlus10k) is False:
        ...
        with open('ctest1pop.xml', encoding='latin-1') as f:
            ...
            try:
                ...
                empiricalOption = round(PopHarvestedAfterAgeAtFirstOffspringPerYrRounded/2)
                percentageOption = (InitPopRounded/2)*.1
                if empiricalOption > percentageOption:
                    SexUnbiasedHarvestEstimate = empiricalOption
                else:
                    SexUnbiasedHarvestEstimate = '(N/2)*.1'
